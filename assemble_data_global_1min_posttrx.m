...
function [eggs, trx] = assemble_data_global_1min_posttrx(trx, egg, substrate, search_dur, chamber_type, min_search)
...
for i = 1:1:eggs.total
    ...
    if(search_dur ~=0)
        if(i > 1 && (eggs.fly(i) == eggs.fly(i-1)))
            tmp_var = max([1, eggs.egg_time(i-1)+120, eggs.egg_time(i)-search_dur*2]);
