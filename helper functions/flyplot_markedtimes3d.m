...
function   out = flyplot_markedtimes3d(trx, eggs, fly, times, xlimits, title_string, dense_ticks_onx)
...
if(eggs.chamber_style == 2)
    ...
    figure; xlabel('Time (hours)'); ylabel('Y position (mm)'); title([title_string ' Fly ' num2str(fly)]); hold on; zlabel('X Position (mm)');                  

    
    c = 'rggbbb';
    [~,b] = max(trx(1,fly).y_mm);
    patch([1, length(trx(1,fly).y_mm), length(trx(1,fly).y_mm), 1], [trx(1,fly).middle_line+barrier_size/2, trx(1,fly).middle_line+barrier_size/2, c_size, c_size],c(trx(1,fly).sucrose(b)+1),'FaceAlpha',.15,'EdgeColor','none');
    [~,b] = min(trx(1,fly).y_mm);
    patch([1, length(trx(1,fly).y_mm), length(trx(1,fly).y_mm), 1], [0,0, trx(1,fly).middle_line-barrier_size/2, trx(1,fly).middle_line-barrier_size/2],c(trx(1,fly).sucrose(b)+1),'FaceAlpha',.15,'EdgeColor','none');
    
    
    fill([1, length(trx(1,fly).y_mm), length(trx(1,fly).y_mm), 1], [trx(1,fly).middle_line-barrier_size/2, trx(1,fly).middle_line-barrier_size/2, trx(1,fly).middle_line+barrier_size/2, trx(1,fly).middle_line+barrier_size/2],[.8 .8 .8],'Linestyle','none');
    plot3 (1:1:length(trx(1,fly).y_mm), trx(1,fly).y_mm,trx(1,fly).x_mm,'k');
    ...
    for j = 1:1:length(a)
        if (eggs.substrate(a(j)) == 0)
            scatter3 (eggs.egg_time(a(j)),trx(1,fly).y_mm(eggs.egg_time(a(j))),trx(1,fly).x_mm(eggs.egg_time(a(j))),15,'r','o','fill');
        end
        if (eggs.substrate(a(j)) == 2)
            scatter3 (eggs.egg_time(a(j)),trx(1,fly).y_mm(eggs.egg_time(a(j))),trx(1,fly).x_mm(eggs.egg_time(a(j))),15,'g','o','fill');
        end
        if (eggs.substrate(a(j)) == 5)
            scatter3 (eggs.egg_time(a(j)),trx(1,fly).y_mm(eggs.egg_time(a(j))),trx(1,fly).x_mm(eggs.egg_time(a(j))),15,'b','o','fill');
        end
    end
end
