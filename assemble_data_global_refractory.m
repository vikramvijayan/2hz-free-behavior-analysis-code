...
function [eggs, trx] = assemble_data_global_refractory(trx, egg, substrate, search_dur, chamber_type, min_search, refractory)
...
for i = 1:1:eggs.total
    ...
    if(search_dur ~=0)
        if(i > 1 && (eggs.fly(i) == eggs.fly(i-1)))
            tmp_var = max([1, eggs.egg_time(i-1)+refractory*2, eggs.egg_time(i)-search_dur*2]);
            if(tmp_var > eggs.egg_time(i))
                tmp_var = eggs.egg_time(i-1);
            end
            eggs.explore_start_time(i) = tmp_var;
        else
